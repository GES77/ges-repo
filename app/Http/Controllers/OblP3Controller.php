...
class OblP3Controller extends Controller
{
    public function p3()
    {
        ...
        $data['getRecord'] = Oblp3::getRecord();
    ...
    public function insert(Request $request)
    {
        ...
        $dokumen->storeAs('p3', $dokumenName);
        Oblp3::create([
        ...
        return redirect('panel/obl/p3')->with('success', 'P3 berhasil ditambah');
    }
    public function edit($id)
    {
        $p3 = Oblp3::findOrFail($id);
        return view('panel.obl.p3.edit', compact('p3'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->hasFile('dokumen')) {
            // Hapus dokumen lama
            $oldDokumenPath = storage_path("app/p3/{$p3->hash_dokumen}");
        ...
        return redirect('panel/obl/p3')->with('success', 'P3 berhasil diperbarui');
    }

    public function download(Oblp3 $dokumen)
    {
        $dokumenPath = storage_path("app/p3/{$dokumen->hash_dokumen}");

        if (file_exists($dokumenPath)) {
            return response()->download($dokumenPath, $dokumen->dokumen);
        } else {
            abort(404, 'P3 tidak ditemukan');
        }
    }

    public function preview($id)
    {
        $dokumen = Oblp3::findOrFail($id);
        $dokumenPath = storage_path("app/p3/{$dokumen->hash_dokumen}");
    ...
    public function delete($id)
    {
        ...
        // Hapus file dokumen
        $dokumenPath = storage_path("app/p3/{$p3->hash_dokumen}");
        if (file_exists($dokumenPath)) {
            unlink($dokumenPath);
        }
        ...
        return redirect('panel/obl/p3')->with('error', 'P3 berhasil dihapus');
    }
}
