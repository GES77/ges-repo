@extends('panel.layouts.app')

@section('content')
    <div class="pagetitle">
        <h3>OBL > P7</h3>

    </div><!-- End Page Title -->

    <section class="section">
        <div class="row">
            <div class="col-lg-12">
                @include('auth._message')
                <div class="card">
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <h5 class="card-title">List P7</h5>
                            </div>
                            <div class="col-md-6" style="text-align: right;">
                                @if (!empty($PermissionAdd))
                                <a href="{{ url('panel/obl/p7/add') }}" class="btn btn-tambah" style="margin-top: 10px;">
                                    <i class="bi bi-plus-square-fill"></i>
                                    <span>Tambah P7</span>
                                </a>
                                @endif
                            </div>
                        </div>

                        <!-- Table with stripped rows -->
                        <table class="table datatable border">
                            <thead>
                                <tr>
                                    <th>Tanggal Dibuat</th>
                                    <th>Nama Uploader</th>
                                    <th>Judul Dokumen</th>
                                    <th>Aksi</th>
                                </tr>
                            </thead>
                            <tbody>
                                @forelse ($getRecord as $value)
                                    <tr>
                                        <td>{{ $value->created_at->format('d M Y') }}</td>
                                        <td>{{ $value->uploader }}</td>
                                        <td>{{ $value->nama_file }}</td>
                                        <td>
                                            @if (!empty($PermissionEdit))
                                                <a href="{{ route('p7.edit', $value->id) }}"
                                                    class="btn btn-edit btn-sm mt-1">Edit</a>
                                            @endif

                                            @if (!empty($PermissionPreview))
                                                <a href="{{ route('p7.preview', $value->id) }}"
                                                    class="btn btn-preview btn-sm mt-1">Preview</a>
                                            @endif

                                            @if (!empty($PermissionDownload))
                                                <a href="{{ route('p7.download', $value) }}"
                                                    class="btn btn-download btn-sm mt-1">Download</a>
                                            @endif

                                            @if (!empty($PermissionDelete))
                                                <a href="{{ route('p7.delete', $value->id) }}"
                                                    class="btn btn-hapus btn-sm mt-1">Hapus</a>
                                            @endif

                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="4" class="text-center">Data masih kosong</td>
                                    </tr>
                                @endforelse

                            </tbody>
                        </table>
                        <!-- End Table with stripped rows -->

                    </div>
                </div>

            </div>
        </div>
    </section>
@endsection
